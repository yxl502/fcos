...
class VOCDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(img.shape)  # torch.Size([3, 832, 1088])
...
if __name__ == "__main__":
    ...
    eval_dataset = VOCDataset(root_dir='../BCCD_Dataset-master/BCCD', resize_size=[800, 1333],
                              split='test', use_difficult=False, is_train=False, augment=None)
    print(len(eval_dataset.CLASSES_NAME))
    dataset = VOCDataset("../BCCD_Dataset-master/BCCD",
    ...
    for i in range(3):
        ...
        # print(img)  # torch.float32

        img, boxes, classes = img, boxes.numpy(), classes.numpy()
        # print(img.dtype)  #torch.float32
        #
        # cv2.imshow('test1', img.permute(1, 2, 0).numpy())
        # cv2.waitKey(0)

        img = img.permute(1, 2, 0).numpy().copy()
        # print(img.shape)  # (832, 1088, 3)
        # print(boxes)
        ...
        for i, box in enumerate(boxes):
            ...
            cv2.putText(img, str(CLASSES_NAME[classes[i]]), pt1, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 6)
        cv2.imshow("test", img)
        if cv2.waitKey(0) == 27:
            break

    # imgs, boxes, classes = eval_dataset.collate_fn([dataset[105], dataset[101], dataset[200]])
    # print(boxes, classes, "\n", imgs.shape, boxes.shape, classes.shape,
    #       boxes.dtype, classes.dtype, imgs.dtype)
    # print(imgs, imgs.dtype)
    # for index, i in enumerate(imgs):
    #     # print(i.dtype)  # torch.float32
    #     # i = i.numpy().astype(np.uint8)
    #     ii = i.permute(1, 2, 0).numpy()
        # print(i.dtype)
        ...
        # cv2.imshow(str(index)+".jpg", ii)
        #
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # cv2.imwrite('cat.png', i)  # cv2保存图片
